...
class VarAndList {
  constructor(runtime) {
    ...
    this._formatMessage = runtime.getFormatMessage({
      "zh-cn": {
        ...
        "qxsckvarandlist.seriVarsToJson":
          "将以 [START] 为开头的所有变量转换为json",
        ...
        "qxsckvarandlist.getListRange":
          "列表 [LIST] 中第 [LEFT] 到 [RIGHT] 项的值",
        "qxsckvarandlist.getValueOfList": "列表 [LIST] 的第 [INDEX] 项",
        "qxsckvarandlist.seriListsToJson":
          "将以 [START] 为开头的所有列表转换为json",
        ...
        "qxsckvarandlist.insertOfList":
          "在列表 [LIST] 的第 [INDEX] 项前插入 [VALUE]",
        "qxsckvarandlist.insertListToList":
          "在列表 [LIST] 的第 [INDEX] 项前插入列表 [LIST2]",
        "qxsckvarandlist.replaceOfList":
          "替换列表 [LIST] 的第 [INDEX] 项为 [VALUE]",
        "qxsckvarandlist.replaceRangeOfList":
          "把列表 [LIST] 的第 [LEFT] 到 [RIGHT] 项都替换为 [VALUE]",
        "qxsckvarandlist.getIndexOfList": "列表 [LIST] 中第一个 [VALUE] 的位置",
        "qxsckvarandlist.getIndexesOfList": "列表 [LIST] 中所有 [VALUE] 的位置",
        "qxsckvarandlist.newGetIndexesOfList":
          "列表 [LIST] 中所有 [VALUE] 的位置",
        ...
        "qxsckvarandlist.sortListRange":
          "以 [CASE] 排序列表 [LIST] 的第 [LEFT] 到 [RIGHT] 项",
        "qxsckvarandlist.isOrder": "列表 [LIST] 是 [CASE] 的吗？",
        "qxsckvarandlist.mapObject":
          "映射对象 [OBJ] 的值到列表 [LIST] ，键到列表 [LIST2]",
        "qxsckvarandlist.associateList":
          "关联列表 [LIST] 为键，[LIST2] 为值，如果长度不同则 [DO]",

        "qxsckvarandlist.unSupportCompile": "⚠️不支持编译",

        "qxsckvarandlist.forEach":
          "⚠️对于从 [LEFT] 到 [RIGHT] 中的每个变量 [VAR]",
        "qxsckvarandlist.forEachList":
          "⚠️对于列表 [LIST] 中从第 [LEFT] 到第 [RIGHT] 项的每个变量 [VAR]",
      },
      en: {
        ...
        "qxsckvarandlist.setListValue":
          "set list [LIST] to [VALUE] [NUM] times",
        ...
        "qxsckvarandlist.getIndexOfList":
          "first index of [VALUE] in list [LIST]",
        "qxsckvarandlist.getIndexesOfList": "indexes of [VALUE] in list [LIST]",
        "qxsckvarandlist.newGetIndexesOfList":
          "indexes of [VALUE] in list [LIST]",
  ...
  getInfo() {
    return {
      ...
      menuIconURI: Icon,
      docsURI:
        "https://learn.ccw.site/article/647a5fb4-4f01-4b5d-9177-8cf763f68c69",
      blocks: [
    ...
    try {
      if (this.runtime._editingTarget.isStage) {
        ...
        Object.keys(temp).forEach((obj) => {
          if (temp[obj].type === "")
            list.push({ text: temp[obj].name, value: temp[obj].id });
        });
      } else {
    ...
    if (variable) {
      try {
        ...
        for (let i = 0; i < arr.length; i++)
          variable.value.push(JSON.stringify(arr[i]));
        variable._monitorUpToDate = false;
      } catch (error) {
        console.log("error:", error);
      }
